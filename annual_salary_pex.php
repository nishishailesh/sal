...
function vlist_annual_salary($link,$staff_id,$fyear,$fmonth,$tyear,$tmonth)
{
	...
	echo '<table border=1><tr><td>Bill Group</td>';
	$bill_group=array();
	while($bg=mysqli_fetch_assoc($resultt))
	{ 
		echo '<td>'.$bg['bill_group'].'</td>';
		$bill_group[]=$bg['bill_group'];
	}
	echo '</tr>';
	...
	{
		$pmn=find_sums($link,$staff_id,$value);
		echo '<th>'.$pmn[0].'</th>';
	}
	echo '</tr>';

	echo '<tr><th>Deduction</th>';
	foreach($bill_group as $value)
	{
		$pmn=find_sums($link,$staff_id,$value);
		echo '<th>'.$pmn[1].'</th>';
