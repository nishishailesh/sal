...
if($_POST['outer_front']=='general')
{
	$img_file_front='jpg/outer_front_general.jpg';
}
elseif($_POST['outer_front']=='rome')
{
	$img_file_front='jpg/outer_front_rome.jpg';
}
elseif($_POST['outer_front']=='blindness')
{
	$img_file_front='jpg/outer_front_blind.jpg';
}
elseif($_POST['outer_front']=='blank')
{
	$img_file_front=null;
	$GLOBALS['net_for_back']=0; //not required in new format , actually calculated in print_table, but not in print_table_blank
}

if($_POST['outer_back']=='general')
{
	$img_file_back='jpg/OUTER_REGULAR_PAY_BILL-1.jpg';
}
elseif($_POST['outer_back']=='special')
{
	$img_file_back='jpg/outer_special.jpg';
}
elseif($_POST['outer_back']=='ltc')
{
	$img_file_back='jpg/outer_ltc.jpg';
}
elseif($_POST['outer_back']=='blank')
{
	$img_file_back='jpg/praman.png';
}
elseif($_POST['outer_back']=='Fix Pay')
{
	$img_file_back='jpg/fix_pay.jpg';
}
elseif($_POST['outer_back']=='DA Diff')
{
	$img_file_back='jpg/DA_diff.jpg';
}




function outer_back($pdf,$link,$net,$bg,$img_file)
{
	if($img_file!='jpg/praman.png')
	{
	...
	{
		$pdf->Image($img_file, 170, 20, 0, 0, 'PNG', '', '', false, 300, '', false, false, 0);	
...
$pdf->AddPage();
outer_front($pdf,$img_file_front);

$pdf->AddPage();

if($img_file_front==null)
{
	print_table_blank($pdf,$link,$_POST['bill_group'],$_POST['bill_number']);
}
else
{
...
outer_back($pdf,$link,$GLOBALS['net_for_back'],$_POST['bill_group'],$img_file_back);
...
function outer_front($pdf,$img_file)
{
...
function print_plus_tbl_hd()
{
		...
		<th width="4%"><b>(128)<br>NPA</b></th>
		<th width="6%"><b>(002,108)<br>LS/LE,BON</b></th>
		<th width="5%"><b>(005)DA<br>ROP-GZ<br>ROP_NZ</b></th>
		...
		<th width="8%"><b>Gross<br>Amt.</b></th>
		<th width="4%"><b>For Audit<br>Only</b></th>
...
function print_table($pdf,$link,$bg,$bn)
{
	...
	foreach($s as $sr=>$staff_id)
	{
		...
		$pays=get_nsfval($link,$bg,$staff_id,$GLOBALS['pays_id']);
		...
		$bonus=get_sfval($link,$bg,$staff_id,$GLOBALS['bonus']);
		if(!isset($GLOBALS['sm'][$pg]['bouns'])){$GLOBALS['sm'][$pg]['bonus']=0;}
		$GLOBALS['sm'][$pg]['bonus']=$GLOBALS['sm'][$pg]['bonus']+$bonus['amount'];
		
		//P7 DA ROPGz ROPNonGz
		...
		$rop_gz=get_sfval($link,$bg,$staff_id,$GLOBALS['ROP_GZ']);
		if(!isset($GLOBALS['sm'][$pg]['ROP_GZ'])){$GLOBALS['sm'][$pg]['ROP_GZ']=0;}
		$GLOBALS['sm'][$pg]['ROP_GZ']=$GLOBALS['sm'][$pg]['ROP_GZ']+$rop_gz['amount'];	
		
		$rop_nongz=get_sfval($link,$bg,$staff_id,$GLOBALS['ROP_NONGZ']);
		if(!isset($GLOBALS['sm'][$pg]['ROP_NONGZ'])){$GLOBALS['sm'][$pg]['ROP_NONGZ']=0;}
		$GLOBALS['sm'][$pg]['ROP_NONGZ']=$GLOBALS['sm'][$pg]['ROP_NONGZ']+$rop_nongz['amount'];		
		...
		//main table		
			$pstr=$pstr.'<tr>
					<td>'.$count.'</td>				
					<td>'.$staff['fullname'].'<br>'.$dept['data'].','.$post['data'].'<br>'.$ps['data'].'+'.$ops['data'].','.$pays['data'].'</td>
					<td>'.$gp.',<br>'.$sppa['amount'].',<br>'.$fpa['amount'].','.$infection['amount'].'</td>		
					<td>'.$basic.'</td>						
					<td>'.$npa['amount'].'</td>				
					<td>'.$ls['amount'].'<br>,'.$bonus['amount'].'</td>				
					<td>'.$da['amount'].'<br>'.$rop_gz['amount'].'<br>'.$rop_nongz['amount'].'</td>				
					<td>'.$hra['amount'].'</td>				
					<td>'.$cla['amount'].'</td>				
					<td>'.$ma['amount'].'</td>				
					<td>'.$ta['amount'].'</td>				
			...
			if(	$count%$GLOBALS['rpp']==0 || $count==$total_row)
			{
				$pstr=$pstr.'<tr>
						...
						<td>'.($GLOBALS['sm'][$pg]['ls']+$GLOBALS['sm'][$pg]['bonus']).'</td>
						<td>'.($GLOBALS['sm'][$pg]['da']+$GLOBALS['sm'][$pg]['ROP_GZ']+$GLOBALS['sm'][$pg]['ROP_NONGZ']).'</td>
...
if($count>$GLOBALS['rpp'])
{
	...
	foreach($GLOBALS['sm'] as $rn=>$r)
	{
			$sm_str=$sm_str.'<tr>
				...
				<td>'.$GLOBALS['sm'][$rn]['ls'].'<br>,'.$bonus['amount'].'</td>	
				<td>'.($GLOBALS['sm'][$rn]['da']+$GLOBALS['sm'][$rn]['ROP_GZ']+$GLOBALS['sm'][$rn]['ROP_NONGZ']).'</td>
	...
	$sm_str=$sm_str.'<tr>
				...
				<td>'.array_sum(array_column($GLOBALS['sm'],'bonus')).'</td>	
				
				<td>'.
						(array_sum(array_column($GLOBALS['sm'],'da'))+
						array_sum(array_column($GLOBALS['sm'],'ROP_GZ'))+
						array_sum(array_column($GLOBALS['sm'],'ROP_NONGZ'))).'</td>	
	...
	write_text($pdf,array_sum(array_column($GLOBALS['sm'],'ROP_GZ')),157,185,20,5);
	write_text($pdf,array_sum(array_column($GLOBALS['sm'],'ROP_NONGZ')),157,189,20,5);
...
function print_table_blank($pdf,$link,$bg,$bn)
{
	//$s=get_staff_of_a_bill_number($link,$bg,$bn);
	$s=get_staff_of_a_bill_number_namewise($link,$bg,$bn);
	$total_row=count($s);
	...
	foreach($s as $sr=>$staff_id)
	{
	   ...
	   $bonus=get_sfval($link,$bg,$staff_id,$GLOBALS['bonus']);
		if(!isset($GLOBALS['sm'][$pg]['bonus'])){$GLOBALS['sm'][$pg]['bonus']=0;}
		$GLOBALS['sm'][$pg]['bonus']=$GLOBALS['sm'][$pg]['bonus']+$bonus['amount'];
...
if($count>$GLOBALS['rpp'])
{
	...
	write_text_big_fill($pdf,'Bill Number:'.$_POST['bill_group'].'-'.$_POST['bill_number'],148,56,100,12);
	$bill_details=get_raw($link,'select * from bill_group where bill_group=\''.$bg.'\'');
	write_text_big_fill($pdf,'Bill Detail:'.$bill_details['remark'],148,76,100,10);
}
